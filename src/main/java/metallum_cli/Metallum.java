...
public class Metallum {
    ...
    private static void printRows(Element row) {
        Elements row_values = (row.getElementsByTag("td").size() > 0)
                ? row.getElementsByTag("td")
                : row.getElementsByTag("th");
        List<String> table_values = row_values.stream().map(value -> value.text()).toList();
        System.out.format(
                "%32s%32s%32s%32s\n",
                table_values.get(0), table_values.get(1), table_values.get(2), table_values.get(3));
    }
